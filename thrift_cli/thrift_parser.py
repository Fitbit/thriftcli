...
class ThriftParser(object):
	...
	def __init__(self):
		...
		self.endpoints_regex = re.compile(r'^[\r\t ]*(oneway)?\s*(\w+)\s*(\w+)\(([a-zA-Z0-9: ,]*)\)', flags=re.MULTILINE)
		self.fields_regex = re.compile(r'^[\r\t ]*([\d+]):\s*(optional|required)?\s*(\w+)?\s*(\w+)(?:\s*=\s*([^,\s]+))?', flags=re.MULTILINE)
	...
	def _parse_structs(self):
		...
		fields_by_name = {name:self._parse_fields_from_struct_definition(definition) for name, definition in definitions_by_name.items()}
	...
	def _parse_services(self):
		...
		endpoints_by_name = {name:self._parse_endpoints_from_service_definition(definition) for name, definition in definitions_by_name.items()}
